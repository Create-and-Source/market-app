        ...
        <div className="card-grid">
          {list.map(m => (
            <div key={m.id} className="card market-card" onClick={() => onSelect(m.id)}>
              ...
              {m.date < today() && m.notes && (
                <div className="market-card-verdict verdict-${getVerdict(m)}">
                  <AlertCircle size={16} />
...
// ─── Market Detail ────────────────────────────────────────
function MarketDetail({ market, onBack, onEdit, onDelete, products, templates, onUpdateMarket, onPhotoUpload, onPhotoDelete, onLightbox, getProfit, getVerdict }) {
  ...
  // Previous times at this market
  const previousVisits = [] // Would search by market name
            ...
            <div className="detail-section">
              ...
              <div className="card">
                <div className="profit-breakdown">
                  <div className="profit-row"><span className="label">Revenue</span><span style={{ color: 'var(--green)', fontWeight: 700 }}>+{fmt(market.revenue || 0)}</span></div>
                ...
                {products.map(p => {
                  ...
                  return (
                    <div key={p.id} className="checklist-item" style={{ borderColor: 'var(--border)' }}>
                      ...
                      <span className="checklist-item-text">{p.name}</span>
          ...
          <div className="photo-grid">
            <label className="photo-upload">
              ...
              <input type="file" accept="image/*" capture="environment"
                onChange={e => handlePhotoUpload(e, market.id, photoLabel)}
                ref={photoInputRef} />
            </label>
    ...
    <>
      ...
      {filtered.length === 0 && products.length === 0 ? (
        <div className="empty-state">
          ...
          <h3>No products yet</h3>
          <p>Add the products you sell at markets. Track what you bring, what sells, and what people are interested in.</p>
          <button className="btn btn-primary" onClick={onAdd}><Plus size={18} /> Add Product</button>
        </div>
      ) : (
        <div className="card-grid">
            ...
            return (
              <div key={m.id} className="list-item-row" style={{ cursor: 'pointer' }} onClick={() => onSelect(m.id)}>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{fmtShort(m.date)}</div>
                </div>
